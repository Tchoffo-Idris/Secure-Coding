/*
 * =====================================================================
 *  AES-128 (Advanced Encryption Standard) - Simple Demo in C++
 *  No external libraries needed. Pure C++ OOP implementation.
 *
 *  Compile:  g++ aes.cpp -o aes
 *  Run:      ./aes
 * =====================================================================
 *
 *  HOW AES-128 WORKS:
 *  ------------------
 *  AES operates on a 4x4 grid of bytes called the "state" (16 bytes).
 *  Key: 128 bits (16 bytes)  |  Block: 128 bits (16 bytes)  |  Rounds: 10
 *
 *  Encryption steps:
 *    AddRoundKey (with key 0)
 *    For rounds 1 to 9:
 *      1. SubBytes   - each byte replaced using S-Box
 *      2. ShiftRows  - rows shifted cyclically
 *      3. MixColumns - columns mixed using Galois Field math
 *      4. AddRoundKey
 *    Final round (round 10): SubBytes -> ShiftRows -> AddRoundKey
 *
 *  Decryption reverses all steps using inverse operations.
 * =====================================================================
 */

#include <iostream>
#include <iomanip>
#include <cstring>
#include <cstdint>
#include <string>

/* ================================================================
 *  S-BOX AND INVERSE S-BOX
 * ================================================================ */
static const uint8_t SBOX[256] = {
    
};

static const uint8_t INV_SBOX[256] = {
    
};

static const uint8_t RCON[10] = {
    0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36
};

/* ================================================================
 *  AES CLASS
 * ================================================================ */
class AES {
private:
    uint8_t roundKeys[11][16];   // 11 round keys for AES-128

    /* --- Galois Field multiply (used in MixColumns) --- */
    uint8_t gmul(uint8_t a, uint8_t b) const {
        uint8_t p = 0;
        for (int i = 0; i < 8; i++) {
            if (b & 1) p ^= a;
            bool carry = (a >> 7) & 1;
            a <<= 1;
            if (carry) a ^= 0x1B;
            b >>= 1;
        }
        return p;
    }

    /* --- Key Expansion: 16-byte key -> 11 round keys --- */
    void keyExpansion(const uint8_t key[16]) {
        memcpy(roundKeys[0], key, 16);

        uint8_t w[44][4];
        for (int i = 0; i < 4; i++)
            memcpy(w[i], key + 4 * i, 4);

        for (int i = 4; i < 44; i++) {
            uint8_t temp[4];
            memcpy(temp, w[i - 1], 4);

            if (i % 4 == 0) {
                // RotWord: rotate left by 1
                uint8_t t = temp[0];
                temp[0]=temp[1]; temp[1]=temp[2]; temp[2]=temp[3]; temp[3]=t;
                // SubWord: apply S-Box
                for (int j = 0; j < 4; j++) temp[j] = SBOX[temp[j]];
                // XOR with round constant
                temp[0] ^= RCON[i / 4 - 1];
            }
            for (int j = 0; j < 4; j++)
                w[i][j] = w[i - 4][j] ^ temp[j];
        }

        for (int r = 1; r <= 10; r++)
            for (int i = 0; i < 4; i++)
                memcpy(roundKeys[r] + 4 * i, w[r * 4 + i], 4);
    }

    /* --- Step 1: AddRoundKey - XOR state with round key --- */
    void addRoundKey(uint8_t state[16], int round) const {
        for (int i = 0; i < 16; i++)
            state[i] ^= roundKeys[round][i];
    }

    /* --- Step 2: SubBytes - replace each byte via S-Box --- */
    void subBytes(uint8_t state[16]) const {
        for (int i = 0; i < 16; i++)
            state[i] = SBOX[state[i]];
    }

    void invSubBytes(uint8_t state[16]) const {
        for (int i = 0; i < 16; i++)
            state[i] = INV_SBOX[state[i]];
    }

    /*
     * --- Step 3: ShiftRows ---
     * State is stored column-major. Rows are shifted left:
     *   Row 0: no shift
     *   Row 1: shift left by 1
     *   Row 2: shift left by 2
     *   Row 3: shift left by 3
     */
    void shiftRows(uint8_t state[16]) const {
        uint8_t t;
        // Row 1: shift left 1
        t=state[1]; state[1]=state[5]; state[5]=state[9]; state[9]=state[13]; state[13]=t;
        // Row 2: shift left 2
        t=state[2]; state[2]=state[10]; state[10]=t;
        t=state[6]; state[6]=state[14]; state[14]=t;
        // Row 3: shift left 3 (= right 1)
        t=state[15]; state[15]=state[11]; state[11]=state[7]; state[7]=state[3]; state[3]=t;
    }

    void invShiftRows(uint8_t state[16]) const {
        uint8_t t;
        // Row 1: shift right 1
        t=state[13]; state[13]=state[9]; state[9]=state[5]; state[5]=state[1]; state[1]=t;
        // Row 2: shift right 2
        t=state[2]; state[2]=state[10]; state[10]=t;
        t=state[6]; state[6]=state[14]; state[14]=t;
        // Row 3: shift right 3 (= left 1)
        t=state[3]; state[3]=state[7]; state[7]=state[11]; state[11]=state[15]; state[15]=t;
    }

    /* --- Step 4: MixColumns - mix bytes in each column --- */
    void mixColumns(uint8_t state[16]) const {
        for (int col = 0; col < 4; col++) {
            uint8_t *s = state + col * 4;
            uint8_t s0=s[0], s1=s[1], s2=s[2], s3=s[3];
            s[0] = gmul(2,s0)^gmul(3,s1)^s2        ^s3;
            s[1] = s0        ^gmul(2,s1)^gmul(3,s2)^s3;
            s[2] = s0        ^s1        ^gmul(2,s2)^gmul(3,s3);
            s[3] = gmul(3,s0)^s1        ^s2        ^gmul(2,s3);
        }
    }

    void invMixColumns(uint8_t state[16]) const {
        for (int col = 0; col < 4; col++) {
            uint8_t *s = state + col * 4;
            uint8_t s0=s[0], s1=s[1], s2=s[2], s3=s[3];
            s[0]=gmul(0x0E,s0)^gmul(0x0B,s1)^gmul(0x0D,s2)^gmul(0x09,s3);
            s[1]=gmul(0x09,s0)^gmul(0x0E,s1)^gmul(0x0B,s2)^gmul(0x0D,s3);
            s[2]=gmul(0x0D,s0)^gmul(0x09,s1)^gmul(0x0E,s2)^gmul(0x0B,s3);
            s[3]=gmul(0x0B,s0)^gmul(0x0D,s1)^gmul(0x09,s2)^gmul(0x0E,s3);
        }
    }

public:
    /* Constructor: takes 16-byte key and generates round keys */
    AES(const uint8_t key[16]) {
        keyExpansion(key);
    }

    /* --- Encrypt a 16-byte block --- */
    void encrypt(const uint8_t plaintext[16], uint8_t ciphertext[16]) const {
        uint8_t state[16];
        memcpy(state, plaintext, 16);

        addRoundKey(state, 0);           // Initial key addition

        for (int round = 1; round <= 9; round++) {
            subBytes(state);             // Replace bytes via S-Box
            shiftRows(state);            // Rotate rows
            mixColumns(state);           // Mix columns
            addRoundKey(state, round);   // XOR with round key
        }

        // Final round: no MixColumns
        subBytes(state);
        shiftRows(state);
        addRoundKey(state, 10);

        memcpy(ciphertext, state, 16);
    }

    /* --- Decrypt a 16-byte block --- */
    void decrypt(const uint8_t ciphertext[16], uint8_t plaintext[16]) const {
        uint8_t state[16];
        memcpy(state, ciphertext, 16);

        addRoundKey(state, 10);          // Start with last round key

        for (int round = 9; round >= 1; round--) {
            invShiftRows(state);
            invSubBytes(state);
            addRoundKey(state, round);
            invMixColumns(state);
        }

        // Final inverse round
        invShiftRows(state);
        invSubBytes(state);
        addRoundKey(state, 0);

        memcpy(plaintext, state, 16);
    }
};

/* ================================================================
 *  HELPER: print bytes as hex
 * ================================================================ */
static void printHex(const std::string &label, const uint8_t *data, int len) {
    printf("%-22s: ", label.c_str());
    for (int i = 0; i < len; i++) printf("%02X ", data[i]);
    printf("\n");
}

/* ================================================================
 *  MAIN - Demo
 * ================================================================ */
int main() {
    std::cout << "===========================================\n"
              << "   AES-128 Encryption Demo (Pure C++)     \n"
              << "===========================================\n\n";

    // 16-byte (128-bit) key  (NIST FIPS-197 test vector)
    uint8_t key[16] = {
        43,126, 21, 22, 40,174,210,166,
       171,247, 21,136,  9,207, 79, 60
    };

    // 16-byte (128-bit) plaintext  (NIST FIPS-197 test vector)
    uint8_t plaintext[16] = {
        50, 67,246,168,136, 90, 48,141,
        49, 49,152,162,224, 55,  7, 52
    };

    uint8_t ciphertext[16], decrypted[16];

    printHex("Key (16 bytes)",       key,       16);
    printHex("Plaintext (16 bytes)", plaintext, 16);
    std::cout << '\n';

    // Create AES object with key
    AES aes(key);

    // Encrypt
    aes.encrypt(plaintext, ciphertext);
    printHex("Ciphertext (16 bytes)", ciphertext, 16);

    // Decrypt
    aes.decrypt(ciphertext, decrypted);
    printHex("Decrypted  (16 bytes)", decrypted, 16);

    // Verify
    std::cout << '\n';
    if (memcmp(plaintext, decrypted, 16) == 0)
        std::cout << "SUCCESS: Decrypted text matches original plaintext!\n";
    else
        std::cout << "FAILED: Mismatch!\n";

    std::cout << "\nExpected ciphertext: 39 25 84 1D 02 DC 09 FB DC 11 85 97 19 6A 0B 32\n"
              << "(NIST FIPS-197 official test vector for AES-128)\n";

    return 0;
}
